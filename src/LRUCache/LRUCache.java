		...
		synchronized(hash){
			...
			if (tmpNode != null) {
				...
				if (cacheQueue.Queue.contains(tmpNode)) {
					cacheQueue.Queue.remove(tmpNode);
					// else if the queue is full,then make room for new node
				} else if (cacheQueue.Queue.size() == hash.capasity) {
					cacheQueue.Queue.removeLast();
				}
			} else {
			...
			node.Value = value;
			cacheQueue.Queue.addFirst(node);
		...
		synchronized(hash){
			...
			if (tmpNode != null) {
				if (cacheQueue.Queue.contains(tmpNode)) {
					...
					if (tmpNode != cacheQueue.Queue.getFirst()) {
						cacheQueue.Queue.remove(tmpNode);
					} else {
						return tmpNode;
					}
